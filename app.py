...
def seed_state(state: DTState) -> None:
    ...
    gpu_vram_hints = {
        "gpu:nvidia-a100": 80,
        "gpu:nvidia-h100": 80,
        "gpu:nvidia-rtx4090": 24,
        "gpu:nvidia-rtx6000": 24,
        "gpu:nvidia-rtx3090": 24,
        "gpu:nvidia-rtx3080": 10,
        "gpu:nvidia-rtx3070": 8,
        "gpu:nvidia-tesla-t4": 16,
        "gpu:nvidia-l4": 24,
        "gpu:adreno": 2,
    }

    def infer_gpu_vram(accels: list[str]) -> int:
        for acc in accels:
            key = acc.lower()
            for prefix, vram in gpu_vram_hints.items():
    ...
    for spec in cluster_specs:
        ...
        for node_spec in spec["nodes"]:
            ...
            hardware_cfg = dict(node_spec["hardware"])
            accelerators = list(hardware_cfg.get("accelerators", []))
            gpu_vram = hardware_cfg.get("gpu_vram_gb")
            if gpu_vram is None:
                gpu_vram = infer_gpu_vram(accelerators)
            hw = HardwareSpec(
                cpu_cores=hardware_cfg["cpu_cores"],
                base_ghz=hardware_cfg["base_ghz"],
                memory_gb=hardware_cfg["memory_gb"],
                gpu_vram_gb=gpu_vram,
                arch="amd64",
                tdp_w=hardware_cfg["tdp_w"],
                memory_mhz=hardware_cfg.get("memory_mhz"),
                memory_type=hardware_cfg.get("memory_type"),
