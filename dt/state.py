...
class DTState:
        ...
        def upsert_node(self, node: Node) -> None:
                with self._lock:
                        ...
                        cluster_label = None
                        if node.k8s and node.k8s.labels:
                                cluster_label = node.k8s.labels.get('dt.cluster.name')
                                if cluster_label:
                        ...
                        if cluster_name:
                                ...
                                if not cluster_info:
                                        cluster_type = node.k8s.labels.get('dt.cluster.type') if node.k8s and node.k8s.labels else 'unknown'
                                ...
                                for name, node_dict in self.nodes_by_name.items():
                                        try:
                                                hw = HardwareSpec(
                                                        ...
                                                        memory_mhz=node_dict["hardware"].get("memory_mhz"),
                                                        memory_type=node_dict["hardware"].get("memory_type"),
                                                        accelerators=list(node_dict["hardware"].get("accelerators", [])),
                                                )
                                                rt = NodeRuntime(
        ...
        def zones(self) -> Dict[str, List[Node]]:
                with self._lock:
                        ...
                        for n in self.list_nodes():
                                zone = "unknown"
                                if n.k8s:
                                        zone = n.k8s.zone or n.k8s.labels.get('topology.kubernetes.io/zone', 'unknown')
        ...
        def get_virtual_identity(self, node_name: str) -> Optional[NetworkIdentity]:
                with self._lock:
                        ...
                        return node.network if node else None
