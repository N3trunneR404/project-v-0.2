...
def compute_stage_runtime_ms(
	...
	"""
	Estimate runtime for a stage on a specific node, accounting for ISA overhead
	and workload type.
	"""
...
class DiscreteEventSimulator:
	"""
	Event-driven simulation engine that models queueing, resource contention,
	and (optionally) stochastic node failures for a single job placement.
	"""
	...
	def simulate(self, job: Job, placements: Dict[str, PlacementDecision]) -> DESMetrics:
		...
		self._nominal_duration = sum(max(1.0, float(stage.compute.duration_ms)) for stage in job.stages) or 1000.0
		...
		self._schedule_initial_ready_events(job, placements)
		...
		while self.events:
			...
			if event.event_type == "stage_ready" and event.stage_id:
				self._handle_stage_ready(stage_map[event.stage_id], placements[event.stage_id], event.time_ms)
			elif event.event_type == "stage_complete" and event.stage_id:
				self._handle_stage_complete(stage_map[event.stage_id], placements[event.stage_id])
			elif event.event_type == "node_failure" and event.node_name:
				self._handle_node_failure(event.node_name)

		metrics = self._build_metrics(job)
		return metrics
	...
	def _build_dependency_graph(self, job: Job) -> None:
		for stage in job.stages:
			if stage.id not in self.remaining_deps:
				self.remaining_deps[stage.id] = 0
	...
	def _schedule_initial_ready_events(self, job: Job, placements: Dict[str, PlacementDecision]) -> None:
		for stage in job.stages:
	...
	def _schedule_failures(self, placements: Dict[str, PlacementDecision]) -> None:
		...
		for node_name in nodes:
			if self.rng.random() <= self.failure_rate:
				...
				self._push_event(fraction * self._nominal_duration, "node_failure", node_name=node_name)

	# ----------------------------------------------------------------- events

	def _handle_stage_ready(self, stage: JobStage, decision: PlacementDecision, ready_time: float) -> None:
		queue = self.node_queues[decision.node_name]
		required_cores = max(1, int(stage.compute.cpu))
		entry = _QueueEntry(stage=stage, decision=decision, arrival_ms=ready_time, required_cores=required_cores)
	...
	def _handle_stage_complete(self, stage: JobStage, decision: PlacementDecision) -> None:
		...
		for dependent in self.dependents.get(stage.id, []):
			self.remaining_deps[dependent] = max(0, self.remaining_deps.get(dependent, 0) - 1)
			if self.remaining_deps[dependent] == 0:
				...
				self._push_event(self.clock_ms + required_delay, "stage_ready", stage_id=dependent)
	...
	def _start_stage(self, entry: _QueueEntry, queue: _NodeQueue, start_time: float) -> None:
		node = queue.node
		runtime_ms = compute_stage_runtime_ms(entry.stage, node, entry.decision.exec_format, self.qemu_overhead)
		...
		self._push_event(start_time + duration_ms, "stage_complete", stage_id=entry.stage.id, priority=1)
	...
	def _dependency_delay(self, predecessor_id: str, stage_id: str) -> float:
		...
		if not prev_exec or prev_exec.finish_ms is None:
			return 0.0
		return compute_network_delay_ms(self.state, prev_exec.node_name, next_decision.node_name)
	...
	def _build_metrics(self, job: Job) -> DESMetrics:
		...
		sla_violated = bool(self.failed_stages) or (job.deadline_ms and latency_ms > job.deadline_ms)
	...
	def _push_event(
		...
		*,
		...
		self._event_seq += 1
		event = _Event(time_ms=time_ms, priority=priority, event_type=event_type, stage_id=stage_id, node_name=node_name)
